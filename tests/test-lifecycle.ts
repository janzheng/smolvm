...
const SANDBOX_NAME = "cx04-lifecycle";
...
const tCreate = performance.now();
{
  ...
  const startResp = await apiPost(`/sandboxes/${SANDBOX_NAME}/start`);
  if (!startResp.ok) throw new Error(`Start failed: ${await startResp.text()}`);

  const echo = await sh(SANDBOX_NAME, "echo ready");
  if (echo.exit_code !== 0) throw new Error("Sandbox not ready");
}
...
const tBootstrap = performance.now();
{
  ...
  const install = await sh(SANDBOX_NAME, "apk add --no-cache git curl nodejs npm 2>&1 | tail -1", { timeout_secs: 120 });
  console.log(`  apk: ${install.stdout.trim()}`);

  const gitV = await sh(SANDBOX_NAME, "git --version");
  const nodeV = await sh(SANDBOX_NAME, "node --version");
  const npmV = await sh(SANDBOX_NAME, "npm --version");
  console.log(`  git: ${gitV.stdout.trim()}`);
  console.log(`  node: ${nodeV.stdout.trim()}`);
  console.log(`  npm: ${npmV.stdout.trim()}`);

  await sh(SANDBOX_NAME, "mkdir -p /workspace && cd /workspace && git init && git config user.email 'agent@test' && git config user.name 'Agent'");

  await sh(SANDBOX_NAME, `cat > /workspace/calc.ts << 'TSEOF'
...
TSEOF`);
  ...
  await sh(SANDBOX_NAME, `cat > /workspace/calc_test.ts << 'TSEOF'
...
TSEOF`);
  ...
  await sh(SANDBOX_NAME, "cd /workspace && git add -A && git commit -m 'initial: calc with multiply bug'");
}
...
Return ONLY the fixed function, no explanation:`;
    ...
    if (groqResp.ok) {
      ...
      await sh(SANDBOX_NAME, `cd /workspace && sed -i 's/return 0;/return a * b;/' calc.ts`);
    } else {
  ...
  await sh(SANDBOX_NAME, `cat > /workspace/test.js << 'JSEOF'
const fs = require("fs");
...
JSEOF`);
  ...
  const testResult = await sh(SANDBOX_NAME, "cd /workspace && node test.js");
  console.log(`  Test: ${testResult.stdout.trim()}`);

  await sh(SANDBOX_NAME, "cd /workspace && git add -A && git commit -m 'fix: multiply returns product instead of 0'");
}
...
{
  const diff = await sh(SANDBOX_NAME, "cd /workspace && git diff HEAD~1");
  ...
  const log = await sh(SANDBOX_NAME, "cd /workspace && git log --oneline");
  console.log(`\n  Commits:\n    ${log.stdout.trim().split("\n").join("\n    ")}`);

  const fixed = await sh(SANDBOX_NAME, "cat /workspace/calc.ts");
...
{
  await apiPost(`/sandboxes/${SANDBOX_NAME}/stop`);
  await apiDelete(`/sandboxes/${SANDBOX_NAME}`);
}
