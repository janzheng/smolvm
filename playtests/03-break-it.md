...
- Create a machine with special characters in the name: spaces, unicode,
...
- Create a machine, then create another with the same name. What happens?

## Attack surface: machine escape

- Try to access the host filesystem from inside: `cat /proc/1/environ`,
  `ls /host`, `mount`
- Fork bomb: `:(){ :|:& };:` — does the machine contain it? Does the host
  survive?
- Fill the disk: `dd if=/dev/zero of=/tmp/fill bs=1M count=10000`
- Exhaust memory: `python3 -c "x = 'a' * (1024**3)"`
- Try to access another machine's filesystem from inside one machine
...
- Path traversal: `GET /api/v1/machines/test/files/workspace/../../etc/passwd`
...
- Run a process that never exits (`sleep infinity`). What happens to the exec
  endpoint?
- Open many concurrent exec requests to the same machine

## What we're really testing

- Input validation completeness
- Machine isolation (VM boundary holds)
