...
**Mission**: Use a machine to actually build and run a web application from
...
Create a machine from the `claude-code` or `node` starter. Your job is
to build a small web server inside it and prove it works.

## Things to try

- Create the machine. How long does it take? Does the starter image work?
...
- Install dependencies (`npm install`, `deno cache`, etc). Does it work? How
  fast?
- Start the server. Can you curl it from inside the machine?
...
- Upload a tar back into a fresh machine. Does the project still work?
