...
- **Gateway/Sandbox architecture**: A control-plane "gateway" manages sandbox lifecycle. Sandboxes are the data plane. Gateway can run local (Docker), remote (SSH), or cloud (reverse proxy).
- **Declarative YAML policies**: All security is policy-driven. Filesystem paths (read-only vs read-write), network endpoints (host+port+binary), process identity (unprivileged, no sudo). Network policies are hot-reloadable without restarting sandboxes.
...
- **Privacy/inference routing**: Built-in `inference.local` endpoint routes LLM API calls through a managed proxy that strips sandbox credentials and injects backend credentials. Supports OpenAI, Anthropic, NVIDIA providers.
- **Supported agents**: Claude Code (full), OpenCode (partial), Codex (needs custom policy), OpenClaw (bundled). Community sandbox images in separate repo.
- **Python CLI**: `uv tool install -U openshell` then `openshell sandbox create -- claude`

## Architecture Components

| Component | Role |
|-----------|------|
| **Gateway** | Control-plane API — sandbox lifecycle, auth boundary, request broker |
| **Sandbox** | Isolated runtime — container supervision + policy-enforced egress |
...
| **Architecture** | Client→Gateway→Sandbox (control/data plane split) | REST API on localhost (single binary) |
...
- **Enterprise posture**: Gateway architecture supports remote/cloud deployment, multi-sandbox management, audit trails
...
- **Fleet performance**: 82ms/sandbox parallel start, cross-sandbox parallel exec
...
3. **Hot-reloadable network policies** — change network rules without destroying the sandbox
...
6. **Community sandbox images** — pre-configured environments for popular agents
...
OpenShell's Docker dependency is a meaningful limitation for macOS-native workflows. smolvm's ability to run micro VMs natively on macOS via Hypervisor.framework is a real advantage. But OpenShell's policy engine could potentially be adapted to run in front of smolvm sandboxes.
...
3. Enterprise posture — Gateway architecture, remote/cloud deployment, multi-sandbox management, audit trails.
...
5. Fleet parallelism — 82ms/sandbox parallel start, true cross-sandbox parallel exec.
...
1. **No macOS isolation at all** — confirmed it's a full no-op sandbox on macOS. Only the proxy and policy evaluation run. smolvm is the only option for real macOS isolation.

2. **K3s dependency is heavy** — the whole thing runs as a K3s cluster inside Docker. Opposite of our single-binary philosophy.

3. **No snapshots/clone/diff** — sandboxes are ephemeral K8s pods. No state management whatsoever. This is a major gap vs smolvm.
...
- https://docs.nvidia.com/openshell/latest/sandboxes/index.html (sandboxes)
- https://docs.nvidia.com/openshell/latest/sandboxes/policies.html (policies)
