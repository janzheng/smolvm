...
Antspace is essentially what we're building with smolvm — but owned by Anthropic and tightly coupled to their stack. The architecture validates our approach (Firecracker microVMs for agent sandboxing) and shows what a production version looks like. Key differences: **we own the stack**, we're not locked to one LLM vendor, and we can expose the full platform to users.
...
| WebSocket tunnel to control plane | cloudflared tunnel | Remote access to sandboxes |
...
3. **Session lifecycle hooks** — Pre-stop hooks check for uncommitted git changes, build errors, etc. before allowing teardown. Smart for agent sandboxes too.
...
- **MCP tool exposure** — First-class MCP server integration in sandbox
...
- **Single-tenant focus** — Baku is one-user-one-sandbox. We need multi-agent-per-sandbox and fleet orchestration
...
4. How do we handle MCP server lifecycle inside the sandbox?
...
| Sandbox cloning | Not mentioned | APFS COW clone + diff + merge | **Ahead** |
...
|-----------|-------------------|-----------|----------|
| **NDJSON status streaming** | All long operations stream status via `application/x-ndjson` (packaging → uploading → building → deploying → deployed) | Our API is request/response only. No streaming status for sandbox creation, file uploads, or agent runs. WebSocket exec streams stdout but no structured status. | High |
...
| **MCP server inside sandbox** | First-class MCP servers run inside the VM (supabase, codesign). Agents get tools automatically. | No MCP integration. Agents in sandboxes can't expose or consume MCP tools. | High |
...
| **Resume-cached sessions** | BYOC default is `resume-cached` — fastest restart, state reuse across sessions. | Sandboxes persist but no "session resume" concept. No warm-start optimization. | Medium |
| **Code/binary signing** | MCP codesign server verifies binaries before execution. | No trust verification for code pushed into sandboxes. | Medium |
...
| **Deployment pipeline** | Full deploy flow: build locally → upload tar → stream status → live URL. | No deployment from sandbox. Sandbox is the end state, not a build step. | Low (different use case) |
...
| **Identity/whoami** | `/v1/environments/whoami` — sandbox knows its own identity, owner, permissions. | Sandboxes are anonymous. No identity, no ownership, no RBAC. | Medium |

### What We Have That They Don't

| Capability | smolvm | Why It Matters |
|-----------|--------|---------------|
| **Git-like sandbox workflows** | Clone (APFS COW), diff, merge, snapshots | Fork/merge agent work — critical for multi-agent coordination |
...
| **Fleet orchestration** | fleet up/down/exec/ls, agent fleet | Batch operations across many sandboxes |
| **File sync watch** | Bidirectional sync with debounce | Local dev with sandbox execution |
