...
### 2. Create and Use a Machine

**cURL:**

```bash
# Create a machine with networking
curl -X POST http://localhost:8080/api/v1/machines \
  -H 'Content-Type: application/json' \
  -d '{"name": "dev-01", "resources": {"cpus": 2, "memory_mb": 2048, "network": true}}'

# Start it
curl -X POST http://localhost:8080/api/v1/machines/dev-01/start

# Run a command
curl -X POST http://localhost:8080/api/v1/machines/dev-01/exec \
  -H 'Content-Type: application/json' \
  -d '{"command": ["echo", "hello from the machine"], "timeout_secs": 30}'
...
# Stop and delete
curl -X POST http://localhost:8080/api/v1/machines/dev-01/stop
curl -X DELETE http://localhost:8080/api/v1/machines/dev-01
```
...
const machine = await client.createAndStart("dev-01", {
  cpus: 2,
  memoryMb: 2048,
  network: true,
});

const result = await machine.sh("echo hello && uname -a");
console.log(result.stdout);

await machine.cleanup(); // stop + delete
```
...
machine = await client.create_and_start("dev-01", cpus=2, memory_mb=2048, network=True)

result = await machine.sh("echo hello && uname -a")
print(result.stdout)
...
```typescript
const machine = await client.createAndStart("agent-01", { fromStarter: "claude-code", network: true });
```
...
SDK shortcut: `machine.sh("ls -la /app && echo done")`
...
Run setup commands automatically when the machine is created:
...
```bash
curl -X PUT http://localhost:8080/api/v1/machines/dev-01/files/app%2Fmain.ts \
...
```bash
curl http://localhost:8080/api/v1/machines/dev-01/files/app%2Fmain.ts
...
```bash
curl http://localhost:8080/api/v1/machines/dev-01/files?dir=/app
```

### Delete a File

```bash
curl -X DELETE http://localhost:8080/api/v1/machines/dev-01/files/app%2Fmain.ts
```
...
```bash
curl -X POST http://localhost:8080/api/v1/machines/dev-01/upload/app%2Fdata.bin \
...
await machine.uploadFile("/app/data.bin", data, "0755");
```
...
curl -X POST http://localhost:8080/api/v1/machines/dev-01/archive/upload?dir=/app \
  -H 'Content-Type: application/gzip' \
  --data-binary @./project.tar.gz

# Download /app as tar.gz
curl http://localhost:8080/api/v1/machines/dev-01/archive?dir=/app -o backup.tar.gz
```

```typescript
const archive = await Deno.readFile("./project.tar.gz");
await machine.uploadArchive(archive, "/app");

const backup = await machine.downloadArchive("/app");
...
### Clone (Fork) a Machine

Create an independent copy. Instant on macOS APFS (copy-on-write):

```bash
curl -X POST http://localhost:8080/api/v1/machines/dev-01/clone \
  -d '{"name": "dev-01-fork"}'
```

```typescript
const fork = await machine.clone("dev-01-fork");
...
```typescript
const diff = await machine.diff("dev-01-fork");
// diff.files: [{path: "/app/main.ts", status: "modified"}, ...]
```

### Merge Machinees

Apply changes from one machine to another:

```bash
curl -X POST http://localhost:8080/api/v1/machines/dev-01-fork/merge/dev-01 \
  -d '{"strategy": "theirs"}'
```
...
Save a machine's full state for later:

```bash
curl -X POST http://localhost:8080/api/v1/machines/dev-01/push
```
...
await fleet.execEach((machine, i) =>
...
Restrict which domains a machine can access:
...
```bash
curl -X POST http://localhost:8080/api/v1/machines/dev-01/images/pull \
  -d '{"image": "docker.io/library/alpine:latest"}'
```

### Run a Command in an Image

Ephemeral overlay — machine filesystem is untouched:

```bash
curl -X POST http://localhost:8080/api/v1/machines/dev-01/run \
...
If a machine runs a web server you want publicly accessible, install cloudflared inside the VM:

```typescript
await machine.sh("curl -L https://github.com/cloudflare/cloudflared/releases/latest/download/cloudflared-linux-arm64 -o /usr/local/bin/cloudflared && chmod +x /usr/local/bin/cloudflared");
await machine.sh("cloudflared tunnel --url http://localhost:3000 &");
```
...
```bash
curl http://localhost:8080/api/v1/machines/dev-01/stats
...
```bash
# Check mount status
curl http://localhost:8080/api/v1/machines/dev-01/debug/mounts

# Check network config
curl http://localhost:8080/api/v1/machines/dev-01/debug/network
```
...
| `GET` | `/api/v1/machines/{id}/stats` | Resource statistics |
| **Execution** | | |
| `POST` | `/api/v1/machines/{id}/exec` | Execute command |
| `GET` | `/api/v1/machines/{id}/exec/stream` | WebSocket streaming exec |
| `POST` | `/api/v1/machines/{id}/run` | Run in OCI image |
| **Files** | | |
| `GET` | `/api/v1/machines/{id}/files` | List files |
| `GET` | `/api/v1/machines/{id}/files/{path}` | Read file |
| `PUT` | `/api/v1/machines/{id}/files/{path}` | Write file |
| `DELETE` | `/api/v1/machines/{id}/files/{path}` | Delete file |
| `POST` | `/api/v1/machines/{id}/upload/{path}` | Multipart upload |
| `POST` | `/api/v1/machines/{id}/archive/upload` | Upload tar.gz |
...
| **Images** | | |
| `GET` | `/api/v1/machines/{id}/images` | List pulled images |
| `POST` | `/api/v1/machines/{id}/images/pull` | Pull OCI image |
| **Containers** | | |
| `POST` | `/api/v1/machines/{id}/containers` | Create container |
| `POST` | `/api/v1/machines/{id}/containers/{cid}/start` | Start container |
| `POST` | `/api/v1/machines/{id}/containers/{cid}/exec` | Exec in container |
| `POST` | `/api/v1/machines/{id}/containers/{cid}/stop` | Stop container |
| `DELETE` | `/api/v1/machines/{id}/containers/{cid}` | Delete container |
| **MicroVMs** | | |
...
| **Debug** | | |
| `GET` | `/api/v1/machines/{id}/debug/mounts` | Mount diagnostics |
| `GET` | `/api/v1/machines/{id}/debug/network` | Network diagnostics |
...
Core tenet: **separate interaction from execution.** The agent's computer (memory, files, tools, state) lives inside a smolvm machine. You just connect to it from wherever.
